...
Il firmware esegue due test hardware in loop, riportando i risultati via UART:
...
1. Inizializzazione del sistema (clock, ADC1, ADC2, UART5) tramite `mx_system_init()`.
...
2. Accensione di **un solo LED alla volta**, in ordine DL2 -> DL11 (poi si ripete), con stampa via UART di quale LED è acceso e quale coppia di pin lo pilota.
